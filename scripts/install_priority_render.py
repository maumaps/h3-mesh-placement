...
def render_html_document(
    ...
    html_parts = [
        ...
        ".phase-control{margin:12px 0 14px;padding:12px;border:1px solid #cfd8df;border-radius:10px;background:#f7fafc;}",
        ".phase-control-label{margin:0 0 8px;font-size:0.78rem;text-transform:uppercase;letter-spacing:0.05em;color:#5a6673;font-weight:700;}",
        ".phase-tabs{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;}",
        ".phase-tab{appearance:none;text-align:left;border:1px solid #c9d5df;background:#fff;color:#17324d;border-radius:8px;padding:10px 12px;font:inherit;cursor:pointer;box-shadow:0 1px 4px rgba(23,50,77,0.06);}",
        ".phase-tab strong{display:block;font-size:0.98rem;line-height:1.25;}",
        ".phase-tab span{display:block;margin-top:3px;font-size:0.82rem;line-height:1.3;color:#5a6673;}",
        ...
        ".basemap-control button{width:auto;min-width:64px;height:30px;border:0;border-left:1px solid #d7dee4;background:#fff;color:#17324d;font:700 11px/1 'Trebuchet MS','Segoe UI',sans-serif;padding:8px 10px;cursor:pointer;white-space:nowrap;}",
        ".basemap-control button:first-child{border-left:0;}",
        ".basemap-control button.active,.basemap-control button[aria-pressed='true']{background:#17324d;color:#fff;}",
        ".basemap-control button:focus-visible{outline:2px solid #8fbce8;outline-offset:-2px;}",
        ...
        "<h2>Overview Map</h2>",
        "<div class='phase-control overview-phase-control'>",
        "<p class='phase-control-label'>Map view phase</p>",
        "<div class='overview-view-tabs phase-tabs' role='tablist' aria-label='Overview rollout view'>",
        "<button type='button' class='overview-view-tab phase-tab active' role='tab' aria-selected='true' data-overview-view='connect'><strong>Phase 1: Connect clusters</strong><span>Show only the install prefix that joins the rollout queues.</span></button>",
