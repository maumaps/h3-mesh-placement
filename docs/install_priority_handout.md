...
The HTML output is the primary field artifact and stays as a single file with one interactive MapLibre map.
...
Cluster controls reuse the same top map, swapping its data to the selected cluster and phase instead of creating extra WebGL maps.
...
The shared map includes a fullscreen toggle for field use on smaller screens.
...
Cluster-focused map views use smaller order badges than the overview map so local geometry stays readable on a phone.
The report creates exactly one MapLibre WebGL context, avoiding browser context loss on large handouts.
On phones the shared map uses compact attribution controls so the rollout overlays stay visible without zooming.
...
The SQL plan table applies that country-aware ownership before it chooses a visible-path fallback, so a border bridge only jumps countries when the local country has no reachable seed queue to own it.
...
The HTML file includes a top summary table with one true “next node” per cluster and a shared MapLibre map.
...
The same bootstrap now exposes `window.__installPriorityMaps` for phone-side debugging, which makes it possible to inspect the current shared map and switch views from the console.
