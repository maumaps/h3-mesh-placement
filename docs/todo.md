...
The live tower mix after the safe restart, population-anchor contraction, current wiggle pass, and manually reviewed redundancy anchors is `8 seed + 5 population + 17 route`; use `make db/procedure/mesh_placement_restart` to replay placement stages without rebuilding cached tables.
The manual route redundancy anchors in `data/in/mesh_route_manual_redundancy.csv` are reviewed backup relays for the `100-101`, `102-103`, and `12-99` bridge edges found during DB-first graph review.
