...
The live tower mix after the safe restart, population-anchor contraction, current wiggle pass, and manually reviewed redundancy anchors is `8 seed + 5 population + 27 route`; use `make db/procedure/mesh_placement_restart` to replay placement stages without rebuilding cached tables.
The manual route redundancy anchors in `scripts/mesh_route_manual_redundancy.sql` are reviewed backup relays for the `100-101`, `102-103`, `12-99`, `102` cut-node, `81-95` bridge, `95` cut-node, `94-95` bridge, and `12` cut-node fragility found during DB-first graph review.
